...
QHash<QPair<quint32, quint32>, short> globalAtoms;
...
{
//    short atom = GlobalAddAtomA("SSStats HK " + (globalAtoms.size() + 1));
    Qt::KeyboardModifiers allMods = Qt::ShiftModifier | Qt::ControlModifier | Qt::AltModifier | Qt::MetaModifier;
    key = shortcut.isEmpty() ? Qt::Key(0) : Qt::Key((shortcut[0] ^ allMods) & shortcut[0]);
    mods = shortcut.isEmpty() ? Qt::KeyboardModifiers(0) : Qt::KeyboardModifiers(shortcut[0] & allMods);
    const quint32 nativeKey = nativeKeycode(key);
    const quint32 nativeMods = nativeModifiers(mods);
//    const bool res = registerShortcut(atom, nativeKey, nativeMods);
//    globalAtoms.insert(qMakePair(nativeKey, nativeMods), atom);
...
//    const bool res = unregisterShortcut(globalAtoms.value(qMakePair(nativeKey, nativeMods)));
//    globalAtoms.remove(qMakePair(nativeKey, nativeMods));
